...
function atualizarInterfaceUsuario(user) {
    ...
    if (!authContainer || !appContainer || !userInfoDisplay || !authMessageDisplay) {
    ...
    if (user) {
        ...
        try {
            ...
            console.error('Erro ao carregar dados após autenticação:', error);
            alert('Autenticação realizada, mas houve um erro ao carregar os dados. Verifique o console.');
        }
    } else {
...
async function atualizarTabelaMateriaisInsumos() {
    ...
    try {
        console.log('carregarDados: Iniciando getDocs(collection(db, "materiais-insumos"))'); // LOGGING
        const querySnapshot = await getDocs(collection(db, "materiais-insumos"));
        console.log('carregarDados: getDocs(collection(db, "materiais-insumos")) completado'); // LOGGING
        materiais = [];
...
async function salvarMaoDeObra() {
    ...
    try {
        console.log('carregarDados: Iniciando setDoc(doc(db, "configuracoes", "maoDeObra"), maoDeObraData)'); // LOGGING
        await setDoc(doc(db, "configuracoes", "maoDeObra"), maoDeObraData);
        console.log('carregarDados: setDoc(doc(db, "configuracoes", "maoDeObra"), maoDeObraData) completado'); // LOGGING
...
async function carregarCustosIndiretosPredefinidos() {
    ...
    try {
        console.log('carregarDados: Iniciando getDocs(collection(db, "custos-indiretos-adicionais"))'); // LOGGING
        const querySnapshot = await getDocs(collection(db, "custos-indiretos-adicionais"));
        console.log('carregarDados: getDocs(collection(db, "custos-indiretos-adicionais")) completado'); // LOGGING
...
async function atualizarTabelaProdutosCadastrados() {
    ...
    try {
        console.log('carregarDados: Iniciando getDocs(collection(db, "produtos"))'); // LOGGING
        const querySnapshot = await getDocs(collection(db, "produtos"));
        console.log('carregarDados: getDocs(collection(db, "produtos")) completado'); // LOGGING
        produtos = [];
        querySnapshot.forEach((doc) => {
...
async function atualizarTabelaPrecificacoesGeradas() {
    ...
    try {
        console.log('carregarDados: Iniciando getDocs(collection(db, "precificacoes-geradas"))'); // LOGGING
        const querySnapshot = await getDocs(collection(db, "precificacoes-geradas"));
        console.log('carregarDados: getDocs(collection(db, "precificacoes-geradas")) completado'); // LOGGING
